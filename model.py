...
class DepthToSpaceLayer(Layer):
    def call(self, inputs, scale):
        return tf.nn.depth_to_space(inputs, scale)

class RFDNNet(Model):
    def __init__(self, feat=64, filter_num=64):
    ...
    def RFDB(self, X):
        filter_left = int(X.shape[-1] / 2)
        filter_right = int(X.shape[-1])
    ...
    def main_model(self, X, scale_factor):
        ...
        X_up = Conv2D(self.filter * (scale_factor ** 2), kernel_size=(3,3), padding='same')(LR)
        out = DepthToSpaceLayer()(inputs=X_up, scale=scale_factor)
